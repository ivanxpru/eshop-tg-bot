...
exports.getByLotId = (lot_id) => new Promise((resolve, reject) => {
  db.connect(dbURI, dbName, async (err) => {
    if (err) reject(err);
    const collection = db.get().collection(dbCollection);
...
exports.getByUserId = (user_id, status) => new Promise((resolve, reject) => {
  db.connect(dbURI, dbName, async (err) => {
    if (err) return console.error(err);
    let details;
    const collection = db.get().collection(dbCollection);
...
exports.getByNsuid = (nsuid) => new Promise((resolve, reject) => {
  db.connect(dbURI, dbName, async (err) => {
    if (err) return console.error(err);
    const collection = db.get().collection(dbCollection);
    const details = { nsuid };
...
exports.changeStatus = (lot_id, status) => new Promise((resolve, reject) => {
  db.connect(dbURI, dbName, async (err) => {
    if (err) reject(err);
    const collection = db.get().collection(dbCollection);
    const details = { _id: ObjectId(lot_id) };
