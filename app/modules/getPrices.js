...
setInterval(() => { // Получаем курсы валют раз в сутки
  getData(url_cbr)
    .then((res) => {
      cbr = res;
    })
...
const getPricesRU = (nsuid, discount_b) => new Promise((resolve, reject) => {
      ...
      .then((res) => {
        if (res.prices && res.prices[0].regular_price) {
          if (discount_b && res.prices[0].discount_price) {
            const discount_end_date = new Date(Date.parse(res.prices[0].discount_price.end_datetime));
            ...
            const price_discount_percentage_f = (res.prices[0].discount_price.raw_value / res.prices[0].regular_price.raw_value) * 100;
            response.discount = `Скидка: -${Math.round(price_discount_percentage_f)}% [до: ${day}.${month}.${year}]`;
...
const getPricesEU = (nsuid, discount) => new Promise((resolve, reject) => {
  const response = {};
  response.prices_eu = '';
  (async () => {
      ...
      await getData(url_eu)
        .then((res) => {
          if (res.prices && res.prices[0].regular_price) {
            ...
            const price_eu_regular = Math.round(currency_eu * res.prices[0].regular_price.raw_value);
            if (discount && res.prices[0].discount_price) {
              ...
              response.discount = `Скидка: -${Math.round(price_discount_percentage_f)}% [до: ${day}.${month}.${year}]`;
              response.discount_end_date = discount_end_date;
              const price_eu_discount = Math.round(currency_eu * res.prices[0].discount_price.raw_value);
...
const getPricesNONEU = (nsuid, discount) => new Promise((resolve, reject) => {
  const response = {};
      ...
      await getData(url_noneu)
        .then((res) => {
          if (res.prices && res.prices[0].regular_price) {
            ...
            const currency_noneu = cbr.Valute[currency].Value / cbr.Valute[currency].Nominal;
            const price_noneu_regular = Math.round(currency_noneu * res.prices[0].regular_price.raw_value);
            if (discount && res.prices[0].discount_price) {
              const discount_end_date = new Date(Date.parse(res.prices[0].discount_price.end_datetime));
              const day = discount_end_date.getDate(discount_end_date);
              const month = (`0${discount_end_date.getMonth() + 1}`).slice(-2);
              ...
              response.discount = `Скидка: -${Math.round(price_discount_percentage_f)}% [до: ${day}.${month}.${year}]`;
              response.discount_end_date = discount_end_date;
              const price_eu_discount = Math.round(currency_noneu * res.prices[0].discount_price.raw_value);
...
const getPrices = (nsuid, discount_b) => new Promise((resolve, reject) => {
  if (!cbr) {
