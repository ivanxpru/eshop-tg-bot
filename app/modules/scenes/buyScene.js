...
buyScene
  .enter(async (ctx) => {
    ...
    let message = '';
    if (lots) {
      message = ctx.i18n.t('buy_scene_st0_1', {
        title: ctx.session.state.lot.title,
      });
    } else {
      message = ctx.i18n.t('buy_scene_st0_0', {
        title: ctx.session.state.lot.title,
      });
    }
    options = {
    ...
    await ctx
      .replyWithPhoto(ctx.session.state.lot.boxart_wide, options)
      .catch((err) => {
        console.error(err);
      });
    if (lots) {
      for await (const result of lots) {
        ...
        message += `${ctx.i18n.t('buy_scene_txt0')}: \`${
          result.price
        } ${ctx.i18n.t('buy_scene_txt1')}\`\n`;
        message += `${ctx.i18n.t('buy_scene_txt2')}: \`@${
          user.user.username
        }\``;
        const keyboard = Markup.inlineKeyboard([
          Markup.callbackButton(
            ctx.i18n.t('buy_scene_btn0'),
            `view_${result._id}`,
          ),
        ]);
        options = {
        ...
        await ctx.replyWithPhoto(result.photo, options).catch((err) => {
          console.error(err);
        });
        await delay(1000);
      }
...
buySceneSt1
  .enter(async (ctx, next) => {
    ...
    if (result) {
      ...
      const keyboard = Markup.inlineKeyboard([
        Markup.urlButton(
          ctx.i18n.t('buy_scene_btn1'),
          `https://t.me/${user.user.username}`,
        ),
        Markup.callbackButton(
          ctx.i18n.t('buy_scene_btn2'),
          `lots_${result.user_id}`,
        ),
      ]);
      const options = {
        caption: message,
  ...
  .on('callback_query', async (ctx, next) => {
    ...
    if (ctx.update.callback_query.data.split('_')[0] === 'lots') {
      ctx.session.state.lot.user_id = ctx.update.callback_query.data.split(
        '_',
      )[1];
  ...
  .enter(async (ctx) => {
    ...
    try {
      lots = await getLot.getByUserId(
        Number.parseInt(ctx.session.state.lot.user_id, 10),
        'active',
      );
    } catch {
      lots = null;
    ...
    await ctx.replyWithMarkdown(message).catch((err) => {
      console.error(err);
    });
    if (lots) {
      for await (const result of lots) {
