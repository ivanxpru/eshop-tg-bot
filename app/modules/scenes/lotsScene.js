...
lotsScene
  .enter(async (ctx) => {
    ...
    let lots;
    await ctx
      .replyWithMarkdown(ctx.i18n.t('lots_scene_st0_0'))
      .then((res) => {
    ...
    if (lots.length) {
      ...
      await ctx.telegram.deleteMessage(
        ctx.chat.id,
        ctx.session.state.message.message_id,
      );
      ctx.state.message = await ctx.replyWithMarkdown(
        ctx.i18n.t('lots_scene_st0_1', { lenght: lots.length }),
...
lotsSceneSt1
  .enter(async (ctx) => {
    ...
    lots.forEach(async (lot) => {
      const btnText = `${lot.title} [${lot.price} ${ctx.i18n.t(
        'lots_scene_txt1',
      )}] `;
    ...
    callbackButtons.push([
      Markup.callbackButton(ctx.i18n.t('lots_scene_btn10'), 'back'),
    ]);
    const keyboard = Markup.inlineKeyboard(callbackButtons);
    await ctx.replyWithMarkdown(
      ctx.i18n.t('lots_scene_st1_0'),
      Extra.markup(keyboard),
    );
  })
...
lotsSceneSt2
  .enter(async (ctx) => {
    ...
    const callbackButtons = [];
    try {
      lots = await getLot.getByUserId(
        Number.parseInt(ctx.from.id, 10),
        'active',
      );
    } catch {
      lots = [];
  ...
  .enter(async (ctx) => {
    ...
    if (result) {
      switch (result.status) {
        case 'active':
          callbackButtons.push([
            Markup.callbackButton(ctx.i18n.t('lots_scene_btn2'), 'toArchive'),
          ]);
          callbackButtons.push([
            Markup.callbackButton(ctx.i18n.t('lots_scene_btn3'), 'toSold'),
          ]);
          callbackButtons.push([
            Markup.callbackButton(ctx.i18n.t('lots_scene_btn10'), 'back'),
          ]);
        ...
        case 'archive':
          callbackButtons.push([
            Markup.callbackButton(ctx.i18n.t('lots_scene_btn9'), 'toActive'),
          ]);
          callbackButtons.push([
            Markup.callbackButton(ctx.i18n.t('lots_scene_btn10'), 'back'),
          ]);
      ...
      message += `${ctx.i18n.t('lots_scene_txt0')} ${result.price} ${ctx.i18n.t(
        'lots_scene_txt1',
      )}\n\n`;
      ...
      await ctx.replyWithPhoto(result.photo, options).catch(async (err) => {
